...
- App 支付默认接入 PayPal Checkout；实际收单渠道必须以中国大陆商户主体的开户审核结果为准，原型阶段仍仅模拟支付成功，不发起真实支付。
- 正式支付时，App 向后台创建订单；后台根据订单套餐快照、金额和币种创建 PayPal Checkout 订单并返回托管收银台地址，App 仅负责调起或跳转支付，不直接持有商户密钥。
- PayPal 支付完成后，后台必须以支付平台的异步 Webhook 作为支付成功的唯一依据：校验回调签名、商户订单号、支付金额、币种和订单状态；不得以 App 前端回跳或客户端“支付成功”结果直接开通容量。
...
- 支付订单、支付平台交易号、回调原文、验签结果、处理时间与失败原因需记录，支持对账和问题追溯。
...
3. 设计设备密钥维护
4. 为容量购买接入 PayPal Checkout 或经审核可用的境外收单平台、订单、支付回调验签、对账、过期续费和幂等账本。
